function [A, B, theta, order, solutions] = BuildingMatrix(parameters) 
% BuildingMatrix : produces the matrices to implement the dynamic updating 
%                  of solution as a system of first order linear 
%                  differential equations 
%
%     [A,B,theta,order,solutions]=BuildingMatrix([solution1;solution2])
%     [A,B,theta,order,solutions]=BuildingMatrix([theta;order;memory])
%     [A,B,theta,order,solutions]=BuildingMatrix([solution1;...;solution4])
%     [A,B,theta,order,solutions]=BuildingMatrix([theta,alpha0,alpha1])
%     [A,B,theta,order,solutions]=BuildingMatrix([theta,alpha0,alpha1,alpha2])
%
...
switch l
    ...
    otherwise
        error('Case not implemented, see Help of BuildingMatrix');
end
...
%SpreadTheta : calculating the solutions of the characteristic poly of the 
%              linear differential equation(with constant coefficients) by 
%              spreading 'theta' % among them, providing an Impulsive 
%              Response with the given 'memory' and quickest promptness.
%
%     solutions = SpreadTheta([theta; order; memory])
% 
%     theta: dissipation parameter
%     order: order of the system generating the Impulsive Response
%     memory: approximate desired decay time of the Impulsive Response
%
% Author: Alessandro Rossi (2016)
%         rossi111@unisi.it
...
switch order
    ...
    case 4 % theta=2*sum(solutions) spreated out according to spread_coeffs
        spread_coeffs = [0.6, 0.65, 0.75]; % sum has to be 2 
